...
static void draw_frame(struct engine* engine) {
    ...
    // LOGIKA VISUAL GAME OVER
    if (check_game_over()) {
        glClearColor(0.8f, 0.0f, 0.0f, 1.0f); // Merah
    } else {
        glClearColor(0.1f, 0.1f, 0.15f, 1.0f); // Normal
    }
    ...
    // Player (Biru)
    glScissor((int)(get_player_x() * (float)w) - 60, (int)(0.15f * (float)h), 120, 120);
    glClearColor(0.0f, 0.6f, 1.0f, 1.0f);
    glClear(GL_COLOR_BUFFER_BIT);

    // Musuh (Kuning)
    glScissor((int)(get_enemy_x() * (float)w) - 50, (int)(get_enemy_y() * (float)h), 100, 100);
